...
VERDE_SIGCA = colors.HexColor("#0F6E56")
AZUL_SIGCA  = colors.HexColor("#1E3A8A")
...
def generar_pdf_documento(registro_id: int) -> str | None:
    ...
    try:
        ...
        est_titulo  = ParagraphStyle("titulo", fontName="Helvetica-Bold", fontSize=11,
                                      textColor=AZUL_SIGCA, alignment=TA_CENTER, spaceAfter=4)
        ...
        nombre_asoc = cfg.get("nombre_completo","SIGCA").upper()
        story.append(Paragraph(nombre_asoc, est_titulo))
        story.append(Paragraph(
            f"NIT: {cfg.get('nit','832.001.389-2')} · {cfg.get('municipio','Villeta, Cundinamarca')}",
            est_subtit))
        story.append(HRFlowable(width="100%", thickness=2, color=VERDE_SIGCA, spaceAfter=12))
