...
Índices de rendimiento para las consultas más frecuentes. (#49 timeout/bloqueo)
...
_INDICES = [
    # registro_central — listado y filtros (#49)
...
def migrar(db_path: str = None) -> bool:
    ...
    try:
        for idx_name, idx_target in _INDICES:
            ...
            created += 1
        conn.commit()
        print(f"✅ Migración 019: {created} índices de rendimiento creados/verificados.")
        return True
    except Exception as e:
        conn.rollback()
        _log.error("migrar_019: %s", e, exc_info=True)
        print(f"❌ Error migración 019: {e}")
        return False
    finally:
        conn.close()
