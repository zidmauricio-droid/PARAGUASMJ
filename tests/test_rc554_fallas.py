...
# ── #57-#61 — Editor TipTap guards ───────────────────────────────────────────

def test_editor_no_cdn_en_template():
    """templates/documentos/nuevo.html no debe contener URLs CDN (#56/#61)."""
    import pathlib
    src = pathlib.Path(__file__).parent.parent / "templates" / "documentos" / "nuevo.html"
    code = src.read_text(encoding="utf-8")
    for cdn in ("cdn.jsdelivr.net", "unpkg.com", "cdnjs.cloudflare.com"):
        assert cdn not in code, f"CDN '{cdn}' encontrado en nuevo.html — viola offline-first"


def test_editor_init_guard_presente():
    """nuevo.html debe tener guard _editorInitialized para evitar doble init (#59)."""
    import pathlib
    src = pathlib.Path(__file__).parent.parent / "templates" / "documentos" / "nuevo.html"
    code = src.read_text(encoding="utf-8")
    assert "_editorInitialized" in code, "Guard de doble inicialización ausente"


def test_editor_setEditable_presente():
    """nuevo.html debe llamar editor.setEditable(true) post-init (#57)."""
    import pathlib
    src = pathlib.Path(__file__).parent.parent / "templates" / "documentos" / "nuevo.html"
    code = src.read_text(encoding="utf-8")
    assert "setEditable(true)" in code, "editor.setEditable(true) ausente en nuevo.html"


def test_fallback_editor_no_menciona_internet():
    """fallbackEditor() no debe mencionar 'internet' (#56)."""
    import pathlib
    src = pathlib.Path(__file__).parent.parent / "templates" / "documentos" / "nuevo.html"
    code = src.read_text(encoding="utf-8")
    assert "requiere conexión a internet" not in code, \
        "fallbackEditor menciona internet — viola offline-first"
