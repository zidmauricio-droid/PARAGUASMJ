...
class TestValidaciones(unittest.TestCase):
    ...
    def test_importe_cero_rechazado(self):
        with self.assertRaises(sqlite3.IntegrityError):
            self.conn.execute("INSERT INTO caja_chica (fecha,concepto,tipo_mov,importe) VALUES (?,?,?,?)",
    ...
    def test_codigo_banco_duplicado_rechazado(self):
        with self.assertRaises(sqlite3.IntegrityError):
            self.conn.execute("INSERT INTO bancos (codigo_cuenta,banco_nombre) VALUES (?,?)",
                              ("BCO-001", "Banco Duplicado"))

    def test_banco_saldo_inicial_negativo_permitido_en_bd(self):
        self.conn.execute("INSERT INTO bancos (codigo_cuenta,banco_nombre,saldo_actual) VALUES (?,?,?)",
                          ("BCO-NEG", "Banco Negativo", -1000))
        saldo = self.conn.execute("SELECT saldo_actual FROM bancos WHERE codigo_cuenta='BCO-NEG'").fetchone()[0]
...
class TestSaldoAnt(unittest.TestCase):
    ...
    def test_saldo_anterior_caja_correcto(self):
        ...
        saldo = _saldo_anterior(self.conn, "caja_chica", "2026-01-20")
        self.assertAlmostEqual(saldo, -25000.0)

    def test_saldo_anterior_caja_inicio(self):
        from routes.finanzas import _saldo_anterior
        saldo = _saldo_anterior(self.conn, "caja_chica", "2026-01-01")
    ...
    def test_movimientos_con_saldo_acumulado(self):
        ...
        movs, saldo_ant, total = _movimientos_con_saldo(self.conn, "caja_chica", "2026-01-01", "2026-12-31")
...
class TestIntegridadFinanciera(unittest.TestCase):
    ...
    def test_egreso_atomico_insert_update(self):
        saldo_inicial = float(self.conn.execute(
            "SELECT saldo_actual FROM bancos WHERE codigo_cuenta='BCO-001'").fetchone()[0])
        self.conn.execute("BEGIN IMMEDIATE")
        self.conn.execute(
            "INSERT INTO movimientos_financieros (fecha,fk_banco_id,tipo_mov,concepto,importe,usuario) VALUES (?,?,?,?,?,?)",
            ("2026-06-01", 1, "EGRESO", "Pago prueba", 100000, "admin"))
        self.conn.execute("UPDATE bancos SET saldo_actual=saldo_actual-? WHERE pk_banco_id=1", (100000,))
        self.conn.commit()
        saldo_nuevo = float(self.conn.execute("SELECT saldo_actual FROM bancos WHERE pk_banco_id=1").fetchone()[0])
    ...
    def test_rollback_ante_error(self):
        try:
            self.conn.execute("BEGIN IMMEDIATE")
            self.conn.execute(
                "INSERT INTO movimientos_financieros (fecha,fk_banco_id,tipo_mov,concepto,importe) VALUES (?,?,?,?,?)",
                ("2026-06-01", 1, "EGRESO", "Test rollback", 50000))
            self.conn.execute(
                "INSERT INTO movimientos_financieros (fecha,tipo_mov,concepto,importe) VALUES (?,?,?,?)",
                ("2026-06-01", "INVALIDO", "Forzar error", 1000))
            self.conn.commit()
        except Exception:
            self.conn.rollback()
        n_movs = self.conn.execute(
            "SELECT COUNT(*) FROM movimientos_financieros WHERE concepto='Test rollback'").fetchone()[0]
    ...
    def test_banco_inactivo_no_deberia_recibir_movimientos(self):
        ...
        self.conn.commit()
        status = self.conn.execute("SELECT status FROM bancos WHERE pk_banco_id=1").fetchone()[0]
...
class TestAuditoria(unittest.TestCase):
    ...
    def test_log_caja_nuevo_tiene_importe(self):
        ...
        log = self.conn.execute("SELECT detalle FROM logs_sistema WHERE accion='CAJA_NUEVO'").fetchone()
    ...
    def test_log_caja_eliminar_tiene_concepto_previo(self):
        ...
        log = self.conn.execute("SELECT detalle FROM logs_sistema WHERE accion='CAJA_ELIMINAR'").fetchone()
    ...
    def test_log_banco_inactivar_tiene_saldo(self):
        ...
        log = self.conn.execute("SELECT detalle FROM logs_sistema WHERE accion='BANCO_INACTIVAR'").fetchone()
...
class TestSeguridad(unittest.TestCase):
    ...
    def _skip_si_app_no_disponible(self):
        if not self._app_disponible:
            self.skipTest(f"App no disponible: {self._skip_reason}")
...
class TestResiliencia(unittest.TestCase):
    ...
    def test_paginacion_offset_mas_alla_del_total(self):
        ...
        movs, _, total = _movimientos_con_saldo(
            self.conn, "caja_chica", "2026-01-01", "2026-12-31", limit=10, offset=9999)
        self.assertEqual(movs, [])
        self.assertEqual(total, 2)
